 ...
 *
 * The first process read the file, construct the adjacency matrix and distribute the items to all the other processes.
 *
 * Also each process will get an array that contains the global pageranks. Also every process will get another array
 * that maps the items in the global pageranks array list items to its corresponding url keys.
 *
 * The algorithm should run for fixed number of iterations provided by user or until the difference of pageranks
 * between two consecutive iterations become less than a user defined threshold.
 *
 */
public class MPIPageRank {
    /**
     * This has values from
     * key : values
     * 0 :
     * 1 : 2
     * 2 : 1
    ...
    /**
     * Global ranks of URLs, The key for the rank is kept in the globalKeys array.
     * the URL key for the globalRanks[0] th item is in the globalKeys[0] th item.
     */
    private double globalRanks[];

    /**
     * Global keys, we are using a global ranks array we should keep the keys to array index mapping.
     *
     */
    ...
    /**
     * Global dangling nodes, all nodes with out degree of zero are placed here 
     */
    // private int globalDangling[];

    /**
     * Total number of dangling nodes    
    */
    //   private int totalDangling = 0;
     ...
     */
    public MPIPageRank(String inputFile, String outputFile, double delta, double dampingFactor, int iterations) {
	...
	//ArrayList<Integer> danglingList = null;
        if (rank == 0) {
            globalAdjMat = readAdjMatrix(inputFile);
            totalUrls = globalAdjMat.size();
            totalNumBuf[0] = totalUrls;
	    //danglingList = makeDanglingList(globalAdjMat);
	    //totalDangling = danglingList.size();
	    //COME HERE~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        }
	...
	//globalDangling = new int[totalDangling];
	    ...
	    // for (int i = 0; i < totalDangling; i++) {
	    //globalDangling[0] = danglingList.get(i);
	    //}
        }
	...
	//broadcast the dangling nodes
	//MPI.COMM_WORLD.Bcast(globalDangling, 0, totalDangling, MPI.INT, 0);
        ...
        if (rank == 0) {
            ...
            count = 0;
            // we are going to create a two dimentional arraylist from the input, because it
            // makes our life easier when sending the values to different processes.
            ...
            for (int i = 1; i < nproc; i++) {
                ...
                // now send the keys
                MPI.COMM_WORLD.Send(keys, startIndex, numberOfUrls, MPI.INT, i, 0);
                ...
                for (int j = 0; j < numberOfUrls; j++) {
                    ...
                    MPI.COMM_WORLD.Send(values[startIndex + j], 0, values[startIndex + j].length, MPI.INT, i, 0);
                }
    ...
    /**
     * Compute the page ranks. Here is a rough outline of what you need to do. You may choose a different path than this.
     *
     * The algorithm has a big loop that continues for fixed number iterations or until the
     * difference in two consecutive pagerank calculations are below delta.
     *
     * Inside the loop..
     *
     * First we need to keep the current pageranks, we will use this to compute the difference
     * between current one and the next one so that we can terminate
     *
     * Then we are going to calculate the local page ranks as in sequential pagerank, but we are
     * only going to use the local adjacency matrix and global pageranks. Note you don't need to calculate the (1-d)/N + d * part in this step.
     *
     * After this step we are going to add up all the dangling values across all the processes
     *
     * then we need to update the global pageranks with all the values computed in the local processes
     *
     * after this we can calculate the actual pageranks by updating the values with (1-d)/N + d *
     *
     * Now calculate the difference between old values and new values and terminate if less than delta
	 *
	...
	while((iters < iterations) && (delta < dif[0])) {
	    dif[0] = 0.0;
	    iters++;
	    
	    double totalArray[] = new double[totalUrls];
	    double sum[] = new double[totalUrls];
	    int rank = MPI.COMM_WORLD.Rank();

	    for(Integer page: localAdjMat.keySet()){
		ArrayList<Integer> outlinks = localAdjMat.get(page);
		double outdegree = (double)outlinks.size();
		if(outdegree == 0.0){
		    for(int k = 0; k < globalKeys.length; k++){
			totalArray[k] += globalRanks[getRankIndex(page)]/(double)totalUrls;
		    }
		}
		else{
		    for(int k: outlinks){
			totalArray[getRankIndex(k)] += globalRanks[getRankIndex(page)]/outdegree;
		    }
		}
	    }

	    MPI.COMM_WORLD.Allreduce(totalArray, 0, sum, 0, totalUrls, MPI.DOUBLE, MPI.SUM);	    
	    if(rank == 0) {
		for(int i = 0; i < totalUrls; i++) {
		    sum[i] = ((1.0 - dampingFactor)/(double)totalUrls) + (dampingFactor * sum[i]);
		    dif[0] += Math.abs(sum[i] - globalRanks[i]);
		}
		globalRanks = copy(sum);
	    }
	    MPI.COMM_WORLD.Bcast((Object)globalRanks, 0, totalUrls, MPI.DOUBLE, 0);
	    MPI.COMM_WORLD.Bcast((Object)dif, 0, dif.length, MPI.DOUBLE, 0);
	}   	    
	return iters; 
    }
    ...
    int getRankIndex(Integer key) {
        ...
        throw new IllegalStateException("This cannot be reached, key should be present");
    }

    /**
     * s
     * Save the ranks to a file as well as output the results to the console.
     */
    private void printRank() {
        ...
        try {
            ...
            System.out.println("Error occurred while opening output file for writing..: " + outputFile);
            e.printStackTrace();
        } finally {
            try {
                if (writer != null) {
	...
	int rank = MPI.COMM_WORLD.Rank();
        if (rank == 0) {
            if (args.length < 8) {
                System.out.println("Usage <input_file_name> <output_file_name> <delta> <damp_factor> <iterations>");
                MPI.Finalize();
                System.exit(-1);
            } else {
                if (args[5] != null) {
        ...
        if (MPI.COMM_WORLD.Rank() == 0) {
            System.out.println("The algorithm ran " + itr + " number of iterations");
        }
	...
	str += list.length-1 + ": " + list[list.length-1] + "]";
	return str;
    }

    public static double[] copy(double[] oldList) {
...
// mpjrun.sh -np 4 MPIPageRank pagerank.input output.txt .001 .85 1
